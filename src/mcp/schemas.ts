...
import { ALIAS_PATTERN, DEFAULT_MAX_ROWS, DEFAULT_POOL_MAX, MAX_AFFECTED_ROWS, MAX_GENERIC_SELECT_LIMIT, MAX_MAX_ROWS, MAX_POOL_MAX } from '../constants.js';
...
export const sqlQuerySchema = z
  .object({
    ...
    max_rows: z.number().int().min(1).max(MAX_MAX_ROWS).default(DEFAULT_MAX_ROWS).describe(`允许返回的最大行数，默认 ${DEFAULT_MAX_ROWS}；通用 SELECT 的字面量 LIMIT 独立限制为 1..${MAX_GENERIC_SELECT_LIMIT}。`),
